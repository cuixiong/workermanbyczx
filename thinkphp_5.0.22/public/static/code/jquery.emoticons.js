...
;(function (factory) {
    ...
    $.emoticons = function(parameter,getApi) {
        ...
        $document.on('click',function(e){
            var $target = $(e.target);
            if(!$target.is('.'+options.triggerCls)){
                closeLayer();
            }
        });
    ...
    //插入文字
    function insertText(_api,str) {
        console.log($('.easyEditor').text().length);
        var editorHtml=$('.easyEditor').html();
        if($('.easyEditor').html()=='<p><br></p>'){
            editorHtml='';
        }
        editorHtml=editorHtml.replace(/<(br).*?>/g, "");
        console.log(editorHtml)
        $('.easyEditor').html(editorHtml+_api.format(str));
        if ($('.easyEditor').text().length > 0) {
